...
import AmenitiesTable from "../../components/Table/AmenitiesTable.js";
...
export default function AmenityList(props) {
  ...
  const editHotelAmenity = (editAmenity) => {
    // console.log("editAmenity payload-->", editAmenity);
    const payload = {
      hotel_id: editAmenity.hotel_id,
      amenities: editAmenity.amenity_name,
      amenity_id: editAmenity.id,
    };
    // console.log("editAmenity payload-->", payload);

    Api.editHotelAmenityDetails(payload)
      .then((res) => {
        if (res.data.status == 1) {
  ...
  // delete category
  const deleteAmenity = (deleteData) => {
    const payload = {
      hotel_id: deleteData.hotel_id,
      amenity_id: deleteData.id,
    };
    // console.log("payload category--->", payload);
    Api.deleteHotelAmenity(payload)
      .then((res) => {
        if (res.data.status == 1) {
          setGetAmenityDataCall(true);
          // console.log(res.data);
      ...
      <GridItem xs={12} sm={12} md={12}>
        <Card>
          <CardHeader color="primary">
            <h4 className={classes.cardTitleWhite}>Amenity List</h4>

            <p className={classes.cardCategoryWhite}></p>
          </CardHeader>
          <CardBody>
            <AmenitiesTable
              getAmenityDataCall={getAmenityDataCall}
              setGetAmenityDataCall={setGetAmenityDataCall}
              addHotelAmenity={addHotelAmenity}
              editHotelAmenity={editHotelAmenity}
