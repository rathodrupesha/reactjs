...
export default function CustomTable(props) {
  ...
  const [viewOrderComplainData, setViewOrderComplainData] = useState({});
  const [openModal, setOpenModal] = useState(false);
  const [tableParams, settableParams] = useState(listing);
  const [orderComplainList, setOrderComplainList] = useState([]);
  ...
  const column = [
    ...
    { title: "Ordered By", field: "order_owner_name" },
    { title: "Order Items", field: "order_items" },
  ...
  const handleShowDetails = (event, rowData) => {
    setViewOrderComplainData(rowData);
  ...
  const getHotelOrderComplainList = () => {
    setLoader(true);
    ...
    Api.orderComplainListing(tableParams, payload)
      .then((res) => {
        setLoader(false);
        // console.log(res);
        if (res && res.data && res.data.data) {
          ...
          res.data.data.rows.map((orders) => {
            orderComplain_list.push({
              complain: orders.comment,
              status: convertStringToCamelCase(orders.status),
              order_items: orders.Order_complain.order_items
                .map((o_items) => o_items.order_meals.name)
                .join(", "),
              order_owner_name:
                orders.Order_complain.order_owner.first_name +
                " " +
                orders.Order_complain.order_owner.last_name,

              createdAt: moment(orders.createdAt).format("LL"),
            });
          });

          setOrderComplainList(orderComplain_list);
      ...
      .catch((err) => {
        ...
        if (err) {
          ...
          // console.log(err, "error----orderComplainList");
        }
      });
  };
  //export csv
  const handleExportCSV = () => {
    ...
    Api.exportOrderComplainCSV(payload)
      .then((res) => {
        setLoader(false);
  ...
  return (
    <div className={classes.tableResponsive}>
      ...
      {/*}  <Viewrequestservice
        ...
        viewOrderComplainData={viewOrderComplainData}
