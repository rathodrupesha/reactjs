...
export default function CustomTable(props) {
  ...
  const [requestComplainList, setRequestComplainList] = useState([]);
  ...
  const getHotelRequestComplainList = () => {
    setLoader(true);
    ...
    Api.requestComplainListing(tableParams, payload)
      .then((res) => {
        setLoader(false);
        // console.log(res);
        if (res && res.data && res.data.data) {
          // console.log("list", res.data.data.rows);
          requestComplain_list = [];
          //   console.log("data---", res.data.data.rows);
          res.data.data.rows.map((requests) => {
            requestComplain_list.push({
              complain: requests.comment,
              status: convertStringToCamelCase(requests.status),
              main_service:
                requests.Complaints_request.hotel_request_main_service.name,
              sub_service:
                requests.Complaints_request.hotel_request_sub_service.name,
              createdAt: moment(requests.createdAt).format("LL"),
            });
          });

          setRequestComplainList(requestComplain_list);
      ...
      .catch((err) => {
        ...
        if (err) {
          ...
          // console.log(err, "error----requestComplainList");
        }
      });
  };
  ...
  const handleExportCSV = () => {
    ...
    Api.exportRequestComplainCSV(payload)
      .then((res) => {
        setLoader(false);
